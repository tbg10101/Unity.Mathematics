...
namespace Unity.Mathematics
{
    ...
    public partial struct quaternion_d
    {
        ...
        public quaternion_d(double x, double y, double z, double w) { value.x = x; value.y = y; value.z = z; value.w = w; }
        public quaternion_d(double4 value)                       { this.value = value; }

        public static quaternion_d identity { get { return new quaternion_d(0.0, 0.0, 0.0, 1.0); } }

        public static implicit operator quaternion_d(quaternion q) { return new quaternion_d(q.value); }
    ...
    public static partial class math
    {
        ...
        public static quaternion_d normalize(quaternion_d q)
        {
            ...
            double len = math.dot(value, value);

            // note we use float4 comparison here because this gives us -1 / 0 which is necessary for select.
            //return select(quatIdentity(), q*rsqrt(len), len > float4(epsilon_normal()));
            value = math.select_d(quaternion.identity.value, value * math.rsqrt(len), len > math.epsilon_normal);
        ...
        {
            double3 axisUnit  = math.normalize(axis);
            double sina = math.sin(0.5f * angle);
            double cosa = math.cos(0.5f * angle);
